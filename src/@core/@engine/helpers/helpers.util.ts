...
import {Helpers, HelpersRegistry} from './helpers.registry'
...
export const callHelper = (registry: HelpersRegistry, name: string, value: any, ...args: any[]) => {
  //return registry.resolve(name)(value, ...args)
}
...
export function resolveTemplate(template: string, context: any): any {
  ...
  for (const filterExpr of filterParts) {
    ...
    if (argMatch) {
      ...
      value = callHelper(context.helpers, helperName, value, ...args)
    } else {
      value = callHelper(context.helpers, filterExpr, value)
    }
  }
...
export function resolveStepTemplates(step: any, context: any) {
  // Context includes all previous steps, plus current step input
  const ctx = {
    ...context,
    output: step.output || {},
    steps: context.steps.map((s: any) => ({
      input: s.input,
      output: s.output,
      data: s.data,
    })),
