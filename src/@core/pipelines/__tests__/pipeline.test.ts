...
describe('pipeline unit tests (new abstractions)', () => {
  ...
  describe('orchestration tests', () => {
    ...
    test('should mutate and return the correct state', async () => {
      const initial = getDefaultPipelineConfig({max: 1, mode: PipelineMode.Chained})
    ...
    test('should set step to failed', async () => {
      ...
      expect(result.steps[0].state).toEqual(PipelineState.Failed)
    })
    ...
    test('complicated chain should work as expected', async () => {
      const testFn1 = async (context: any) => {
        return {data: context.data + '1', additional: 1}
      }

      const testFn2 = async (context: any) => {
        return {data: context.data + '2', additional: 2}
      }

      const failingFn = async (context: any) => {
        throw new Error('failed')
      }

      const testFn3 = async (context: any) => {
        return {data: context.data + '3', additional: 3}
      }

      const pipeline = new Pipeline(getDefaultPipelineConfig({mode: PipelineMode.Chained}))
      const result = await pipeline.orchestrate({data: 'bar'}, testFn1, failingFn, testFn2, testFn3)

      expect(result).toBeDefined()
      expect(result.runs).toHaveLength(3)

      expect(result.steps[0].input).toEqual({data: 'bar'})
      expect(result.steps[0].output).toEqual({data: 'bar1', additional: 1})
    })
  })
})
